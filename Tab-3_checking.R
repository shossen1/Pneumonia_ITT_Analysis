...
df2 <- readRDS("/Users/shakir777/Dropbox/HAPIN/Pneumonia ITT/Data/HAPIN_trt_Pneumonia_Visits.rds") %>% 
...
fun.check("retraction", "c36_retraction", 1) # 33547
fun.check("convulsion", "c36_convulsion", 1) # 23218


fun.check("nodding", "c36_nodding", 1) # 23260 33547
fun.check("indraw", "c36_s_indraw", 1)
...
View(df.c36a %>% filter(hhid %in% c(23260, 33547)) %>% dplyr::select(hhid, contains("nodding"),contains("date")))

View(df.c36 %>% filter(hhid == 23034) %>% dplyr::select(hhid, contains("feed"),contains("date")))
...
fun.check("fever", "c36_Fiebre_cal", 1) 
...
#                   "factor(agecat)", ## Laura will correct the category
